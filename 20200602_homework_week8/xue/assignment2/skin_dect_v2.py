...
class Net(nn.Module):
    def __init__(self):
        ...
        # layers for converting rgb image to gray image
        self.bn0 = nn.BatchNorm2d(3)
        self.conv1 = nn.Conv2d(3, 32, (1,1), stride = 1)
        self.bn1 = nn.BatchNorm2d(32)
        self.conv2 = nn.Conv2d(32,1, (1,1), stride = 1)

        # layers for classifying the grapy image as skin or non-skin
        self.fc = nn.Linear(150*150,1)
        # self.pool = nn.MaxPool2d((150, 150))
        # self.bn2  = nn.BatchNorm2d(1)
        # self.pool1 = nn.MaxPool2d(2)
        # self.conv3 = nn.Conv2d(1,1, (3, 3))
        # self.pool2 = nn.MaxPool2d(2)
        # self.conv4 = nn.Conv2d(1,1, (36, 36))
    def forward(self,x):
        # -- convert rgb image to gray image
        x = self.conv1(x)
        x = F.elu(x)
        x = self.conv2(x)
        new_img = torch.sigmoid(x)

        # -- classify the gray image
        n_img = x.shape[0]
        x = x.view(n_img, -1)
        label = torch.sigmoid(self.fc(x)).squeeze()
...
def train_one_epo(net, data_loader):
    ...
    for x, targets in data_loader:
        ...
        loss = criterion(y, targets.to(torch.float))
        loss.backward()
        losses.append(loss)
        optimizer.step()
...
def train(net, epoches, data_loader_train, data_loader_eval):
    ...
    for epoch in range(epoches):
        ...
        if epoch % 1 == 0:
            ...
            outputFL = f"pred_imgs/eval_pred_{epoch}.png"
            visual_check(modelFL, 'Dataset/validation/pos/' , outputFL)
            outputFL = f"pred_imgs/train_pred_{epoch}.png"
...
def visual_check(networkFL, img_dir, outputFL):
    ...
    for i in range(len(imgFLs)):
        ...
        im_new, y = net(torch.Tensor(im).unsqueeze(0).permute(0,3,1,2))
        #im_new = black_white(im_new, cutoff = 0.5)
...
#- the model with lowest eval loss
...
sys.exit()
networkFL = 'networks/model_epo2.pt'
visual_check(networkFL, 'Dataset/validation/pos/' )
visual_check(networkFL, 'Dataset/test/pos/' )


## check the test set
#img_dir ='Dataset/test'
#pred_dir = f"{img_dir}/pred"
#imgFLs = [f"{img_dir}/{f}" for f in os.listdir(img_dir) if not re.search(f'.pred.jpg', f)]
#
#for i in range(len(imgFLs)):
#    im = imageio.imread(imgFLs[i])
#    plt.imshow(im)
...
#
#    net.eval()
#    pred_label, im_new = net(torch.Tensor(im).unsqueeze(0).permute(0,3,1,2))
#    im_new = im_new.squeeze().to(torch.uint8).numpy()
#    plt.imshow(im_new)
#    plt.show()
#    imageio.imsave(f"{imgFLs[i]}.pred.jpg",im_new)
#    print(f"{imgFLs[i]}.pred.jpg saved")
#
